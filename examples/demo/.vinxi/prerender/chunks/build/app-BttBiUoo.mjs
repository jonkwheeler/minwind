...
const I = (t) => (r) => {
  ...
  let i;
  const u = He(t, e, () => i, { base: a, singleFlight: r.singleFlight, transformUrl: r.transformUrl });
...
function at(t) {
  ...
  const n = createMemo(on(t.routerState.matches, (e, i, u) => {
    ...
    for (let l = 0, w = e.length; l < w; l++) {
      const b = i && i[l], g = e[l];
      u && b && g.route.key === b.route.key ? m[l] = u[l] : (f = false, r[l] && r[l](), createRoot((y) => {
    ...
    return r.splice(e.length).forEach((l) => l()), u && f ? u : (a = m[0], m);
  }));
  return O(() => n() && a)();
}
...
function dt(t) {
  ...
  return I({ signal: [() => a, (n) => Object.assign(a, n)] })(t);
}
...
function ft(t) {
  ...
  return it({ get: r, set({ value: n, replace: e, scroll: i, state: u }) {
    e ? window.history.replaceState(De(u), "", n) : window.history.pushState(u, "", n), ct(decodeURIComponent(window.location.hash.slice(1)), i), Q();
  }, init: (n) => ut(window, "popstate", ke(n, (e) => {
...
var gt = ["<div", ' class="dune small brook glade byte harbor"><header class="small bytes travel far"><!--$-->', '<!--/--><nav class="small bytes willow dog">', '</nav></header><main class="gone with">', '</main><footer class="small bytes travel breeze far dog fern"><span>classname compression, dogfooded</span><button>toggle theme</button></footer></div>'];
function wt(t) {
  return ssr(gt, ssrHydrationKey(), escape(createComponent(Ne, { href: "/", class: "quill a through fox", children: "minwind demo" })), escape(createComponent(Ne, { href: "/about", class: "flag ember", children: "about" })), escape(t.children));
}
function At() {
...
//# sourceMappingURL=app-BttBiUoo.mjs.map
