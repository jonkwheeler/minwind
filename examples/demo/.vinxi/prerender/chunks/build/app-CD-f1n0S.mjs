...
import { N as Ne, q as qe, H as He, O as Oe, C as Ce, z as ze, K as Ke, a as q, e as ee, D as De, Q, P as Pe, k as ke$1 } from './components-CWoJdK_F.mjs';
...
function ae(e) {
  ...
  const r = createMemo(on(e.routerState.matches, (t, i, c) => {
    ...
    for (let l = 0, w = t.length; l < w; l++) {
      const b = i && i[l], g = t[l];
      c && b && g.route.key === b.route.key ? f[l] = c[l] : (m = false, n[l] && n[l](), createRoot((R) => {
        n[l] = R, f[l] = Ke(e.routerState, f[l - 1] || e.routerState.base, O(() => r()[l + 1]), () => {
          var _a;
...
const O = (e) => () => createComponent(Show, { get when() {
  return e();
...
function fe(e = true, n = false, a = "/_server", r) {
  return (t) => {
    ...
    function b(o) {
      const s = w(o);
      if (!s) return;
      const [d, u] = s, A = t.parsePath(u.pathname + u.search + u.hash), v = d.getAttribute("state");
      o.preventDefault(), c(A, { resolve: false, replace: d.hasAttribute("replace"), scroll: !d.hasAttribute("noscroll"), state: v ? JSON.parse(v) : void 0 });
    }
    function g(o) {
      const s = w(o);
      if (!s) return;
    ...
    delegateEvents(["click", "submit"]), document.addEventListener("click", b), e && (document.addEventListener("mousemove", R, { passive: true }), document.addEventListener("focusin", g, { passive: true }), document.addEventListener("touchstart", g, { passive: true })), document.addEventListener("submit", p), onCleanup(() => {
      document.removeEventListener("click", b), e && (document.removeEventListener("mousemove", R), document.removeEventListener("focusin", g), document.removeEventListener("touchstart", g)), document.removeEventListener("submit", p);
    });
  };
}
function me(e) {
  ...
  }, init: (r) => ce(window, "popstate", ke$1(r, (t) => {
...
//# sourceMappingURL=app-CD-f1n0S.mjs.map
