...
export async function compareDirs(
  ...
  if (offPages.length === 0) {
    throw new Error(
      `minwind-compare: no prerendered pages discovered in ${offDir}`,
    );
  }
  if (onPages.length === 0) {
    throw new Error(
      `minwind-compare: no prerendered pages discovered in ${onDir}`,
    );
  }
...
function renderRoute(route: RouteComparison): Array<string> {
  ...
  for (const mismatch of route.styleMismatches.slice(0, 10)) {
    if (mismatch.kind === "element-count") {
      lines.push(
        `    elements: off has ${mismatch.off}, on has ${mismatch.on}`,
      );
